...
The loop itself — *decide, authorize, act, observe, repeat, with guardrails* — is a general infrastructure problem. And general infrastructure does not belong in bespoke product code once you have a framework you can trust, because bespoke code has a compounding cost:
...
In a hand-rolled loop every concern is load-bearing on every other concern. Hardening the retry path means re-testing the review path. Fixing the transcript means re-checking the fallback. It worked. It ran LLooMA in production for months, but every improvement cost a full lap around the tangle.
...
That last one matters for the network's principles too: a host deciding a turn still never learns who you are, and the framework, rather than bespoke orchestrator code, now enforces the pause-and-resume boundary around your private tools.
